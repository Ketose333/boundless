...
- 자동: 조건을 만족하면 자동 적용된다.
- 덱 탐색: 덱에서 조건에 맞는 카드를 찾아 손패로 가져온다.
- 즉시 전개: 덱에서 유닛을 즉시 필드로 전개한다.
- 치유: 유닛 체력을 회복한다.
- 마나 충전: 일시적으로 마나를 획득한다.
- 연쇄: 스택에 올린 뒤 스택 해결 시점에 해결된다.
- 대상 지정: 아군/적군 유닛 중 대상을 정해 적용한다.
- 피해: 유닛 또는 본체에 피해를 준다.
...
- 스택은 `resolve_stack` 이벤트로 1개씩 해결한다.
- 덱 탐색(search_deck_to_hand), 즉시 전개(deploy_from_deck) 처리 후 덱을 다시 셔플한다.
