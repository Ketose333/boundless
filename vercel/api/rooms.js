...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    return send(res, 200, { ok: true, roomId, ownerId: room.ownerId, agents: room.agents, started: !!(room.game || finalGame), joinable: agents.length < 2, game: finalGame || room.game, agentNames });
  }
  ...
  if (action === 'join') {
    ...
    if (room.agents.length === 2 && !room.game) {
      ...
      room.game = initGame(room.roomId, a1, a2, decksByAgent);
    }
  ...
  if (action === 'reset') {
    ...
    const now = Date.now();
    const next = { roomId, ownerId: room.ownerId, agents, game: null, createdAt: room.createdAt || now, updatedAt: now, lastSeen: { [auth.username]: now } };
