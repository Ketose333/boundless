...
function hasKV() { return !!kv && typeof kv.get === 'function' && process.env.KV_REST_API_URL; }
...
async function getUser(username) {
  ...
  if (!u) return null;
  if (hasKV()) return await kv.get(userKey(u));
...
async function createUser(username, password, displayName = '') {
  ...
  if (hasKV()) await kv.set(userKey(u), user);
...
async function createSession(username) {
  ...
  if (hasKV()) await kv.set(sessionKey(token), payload, { ex: SESSION_TTL_SEC });
...
async function getSession(token) {
  ...
  if (!t) return null;
  if (hasKV()) return await kv.get(sessionKey(t));
...
async function deleteSession(token) {
  const t = String(token || '').trim();
  if (!t) return;
  if (hasKV() && typeof kv.del === 'function') await kv.del(sessionKey(t));
