(async function initLayout() {
  ...
  async function showBrandSplashOnce() {
    ...
    logo.style.cssText = [
      'color:#fff',
      'font-family:"Lot","Pretendard","Inter",sans-serif',
      'font-weight:1000',
      'letter-spacing:.08em',
      'line-height:.88',
      'text-align:center',
    ...
    logo.innerHTML = '<span style="display:block;">BOUNDLESS</span><span style="display:block;">PROTOCOL</span>';
  ...
  try {
    ...
    if (me.ok) {
      if (location.pathname === '/login.html') {
        ...
        btn.onclick = async () => {
          if (isLiveGamePage()) {
            const ok = await (window.BP_ALERT?.confirm('진행 중인 매치를 종료하고 로그아웃할까요?', '로그아웃 확인') ?? Promise.resolve(confirm('진행 중인 매치를 종료하고 로그아웃할까요?')));
            if (!ok) return;
          }
